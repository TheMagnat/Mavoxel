...
class Game {
    ...
    public:
        ...
        Game(const mav::Window* window) : window_(window), totalElapsedTime_(0),
            camera(glm::vec3(0, 0, 0)), generator(0, CHUNK_SIZE, VOXEL_SIZE),
        ...
        void input(float deltaTime) {
            ...
            if (window_->isPressed(GLFW_KEY_W)){
                camera.ProcessKeyboard(mav::FORWARD, deltaTime);
                // const mav::SimpleVoxel* collisionVoxel = myWorld.getVoxel(camera.Position.x, camera.Position.y, camera.Position.z);
                // if (collisionVoxel) camera.ProcessKeyboard(mav::BACKWARD, deltaTime);
            }

            if (window_->isPressed(GLFW_KEY_S)){
                camera.ProcessKeyboard(mav::BACKWARD, deltaTime);
                // const mav::SimpleVoxel* collisionVoxel = myWorld.getVoxel(camera.Position.x, camera.Position.y, camera.Position.z);
                // if (collisionVoxel) camera.ProcessKeyboard(mav::FORWARD, deltaTime);
            }

            if (window_->isPressed(GLFW_KEY_A)){
                camera.ProcessKeyboard(mav::LEFT, deltaTime);
                // const mav::SimpleVoxel* collisionVoxel = myWorld.getVoxel(camera.Position.x, camera.Position.y, camera.Position.z);
                // if (collisionVoxel) camera.ProcessKeyboard(mav::RIGHT, deltaTime);
            }

            if (window_->isPressed(GLFW_KEY_D)){
                camera.ProcessKeyboard(mav::RIGHT, deltaTime);
                // const mav::SimpleVoxel* collisionVoxel = myWorld.getVoxel(camera.Position.x, camera.Position.y, camera.Position.z);
                // if (collisionVoxel) camera.ProcessKeyboard(mav::LEFT, deltaTime);
            }
        ...
        void gameLoop(float elapsedTime) {
            ...
            if (!SOLO_CHUNK)
                world.bulkCreateChunk(camera.Position, CHUNK_SIZE * 3, false, &generator);

            //Logic phase
            input(elapsedTime);

            const mav::SimpleVoxel* testVoxel = world.getVoxel(camera.Position.x, camera.Position.y, camera.Position.z);
            if (testVoxel) {
                std::cout << "Found VOXEL ! " << camera.Position.x << " " << camera.Position.y << " " << camera.Position.z << std::endl;
            }
            else {
                std::cout << "NO " << camera.Position.x << " " << camera.Position.y << " " << camera.Position.z << std::endl;
            }
            
            const mav::SimpleVoxel* foundVoxel = world.CastRay(camera.Position, camera.Front);

            sun.setPosition(-800.f, 800.f, 0.f); // Fix position
            // sun.setPosition(0.f, 0.f, 0.0f); // Center position
            // sun.setPosition(cos(tempoTotalTime/5.0f) * 400.f + camera.Position.x, sin(tempoTotalTime/5.0f) * 400.f + camera.Position.y, 0.0f + camera.Position.z); // Simulate a sun rotation
            // sun.setPosition(cos(tempoTotalTime/5.0f) * 400.f, sin(tempoTotalTime/5.0f) * 400.f, 0.0f); // Simulate a sun rotation
            // sun.setPosition(camera.Position.x, camera.Position.y, camera.Position.z); // Light on yourself
            ...
            glm::vec3 newPos = camera.Position + camera.Front * 30.0f;
            ...
            world.draw(camera.Position, 130);
